...
class PerformanceReport:
    ...
    def get_data(self):
        ...
        self.daily_index_pct_change = (
            (self.current_index_value / self.rebased_prices["index"].iloc[-2]) - 1
        ) * 100

        # note these should be calculated properly in future...
        self.weekly_index_pct_change = (
            (self.current_index_value / self.rebased_prices["index"].iloc[-5]) - 1
        ) * 100
        self.monthly_index_pct_change = (
            (self.current_index_value / self.rebased_prices["index"].iloc[-28]) - 1
        ) * 100
    ...
    def create_index(self):
        ...
        rebased_prices = rebase_timeseries(
            self.prices, INDEX_DATE
        )  # normalize dataframe
        rebased_prices_index = rebased_prices.multiply(WEIGHTS, axis=1)  # apply weights
        rebased_prices["index"] = rebased_prices_index.sum(
            axis=1
        )  # sum up weights to get the overall index
    ...
    def plot_performance_chart(self):
        ...
        layout = go.Layout(
            ...
            xaxis=dict(
                range=(
                    [
                        self.rebased_prices.index.max() - pd.DateOffset(months=24),
    ...
    def plot_swarm_plot(self):
        ...
        def get_swarm_coords(returns):
            ...
            ax = sns.swarmplot(x=list(returns.values()))
        ...
        layout = go.Layout(
            ...
            xaxis=dict(range=[-0.1, 0.1], tickformat="%", title="Stock Price Growth"),
            margin={"t": 5},
            height=200,
            shapes=[
                dict(
    ...
    def plot_drawdown(self):
        ...
        data = go.Scatter(
            y=self.index_stats.drawdown.values, x=self.index_stats.drawdown.index
        )
        ...
        layout = go.Layout(
            ...
            xaxis=dict(
                range=(
                    [
                        self.index_stats.drawdown.index.min(),
